...
export class ControladorReserva{
    ...
    async agregarReserva(req, res){
        ...
        try {
            ...
            const diffInDays = Math.floor((fechaDeIngreso - fechaDeExtraccion) / (1000 * 60 * 60 * 24))
            if (diffInDays > 0) {
                if (totalPersonas <= maximoDePersonas) {
                    //console.log(datosHabitacion.valorNoche, diffInDays)
                    let costo = Number(datosHabitacion.valorNoche) * Number(diffInDays)
